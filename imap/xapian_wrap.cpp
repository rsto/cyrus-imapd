        ...
        if (stem_strategy != Xapian::TermGenerator::STEM_NONE) {
            ...
            if (config_getswitch(IMAPOPT_SEARCH_INDEX_LANGUAGE)) {
                ...
                if (reliable && lang != CLD2::UNKNOWN_LANGUAGE) {
                    // Only consider a language that maps to a ISO 639-1 code,
                    // otherwise we can't reverse map it in language filters.
...
xapian_query_new_language(const xapian_db_t *db, const char *lang)
{
    ...
    else if (db->db_versions->lower_bound(6) == db->db_versions->end()) {
        /* If none of the databases supports SLOT_DOCLANGS,
         * there is no point using a LanguagePostingSource. */
        syslog(LOG_INFO, "%s: language search not supported", __func__);
        return (xapian_query_t *) new Xapian::Query(Xapian::Query::MatchAll);
    }
    else {
        std::set<int>::iterator it = db->db_versions->lower_bound(0);
        if (it != db->db_versions->end() && *it < 6) {
            syslog(LOG_INFO, "%s: language search not supported by some subdbs", __func__);
        }
    }
