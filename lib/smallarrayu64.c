...
{
    size_t count = smallarrayu64_size((smallarrayu64_t*)sa);
...
EXPORTED void smallarrayu64_move(smallarrayu64_t *dst, smallarrayu64_t *src)
{
    smallarrayu64_fini(dst);
    *dst = *src;
    memset(&src->spillover, 0, sizeof(arrayu64_t));
    smallarrayu64_fini(src);
}
